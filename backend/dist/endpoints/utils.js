...
exports.createNewPost = exports.validateUser = exports.convertTo24CharHex = void 0;
...
async function validateUser(req, res, next) {
    if (req.originalUrl == '/login') { // verify google token and write user id to session
        ...
        if (document == null) {
            const user = await user_1.User.create(userObjId);
        }
        ...
        if (!req.session.userID || !req.session.userObjectId) {
            ...
            const document = await dbClient.findDbItem(enums_1.COLLECTION.USERS, new mongodb_1.ObjectId(req.session.userObjectId));
            if (document == null) {
