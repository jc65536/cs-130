...
//get post's rating
posts_router.get("/posts/:postId/rating",async (req: Request, res: Response) => {
...
posts_router.post("/posts/:postId/rating", async (req: Request, res: Response) => {
    const post = await Post.fromId(new ObjectId(req.params["postId"]));
...
posts_router.get("/posts/:postId/clothes", async (req: Request, res: Response) => {
    const post = await Post.fromId(new ObjectId(req.params["postId"]));
    res.status(200).json(post.clothes);
});

//add clothes to post
posts_router.post("/posts/:postId/clothes", async (req: Request, res: Response) => {
    const post = await Post.fromId(new ObjectId(req.params["postId"]));
